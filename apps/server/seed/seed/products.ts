...
async function seedProductCost(product: SeedProductData): Promise<void> {
  await prisma.productCostHistory.upsert({
    where: { id: product.costHistoryId },
    ...
    create: {
      ...
      changedById: IDS.USER_ADMIN,
      changedAt: COMMON_PRICE_DATE,
      changeReason: 'Costo inicial de carga',
    },
  });

  await prisma.product.update({
    where: { id: product.id },
    data: { currentCostId: product.costHistoryId },
  });
}
