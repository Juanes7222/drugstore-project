...
export class FiscalDocumentsService {
  ...
  async retry(
    ...
    return this.prisma.$transaction(async (tx: any) => {
      ...
      if (!doc) {
        throw new DocumentNotRetryableException(fiscalDocumentId, 'NOT_FOUND');
      }
      ...
      switch (doc.fiscalState) {
        ...
        case 'REJECTED': {
          ...
          if (doc.purchaseReceptionId) {
            const result = await this.createPendingDocumentForPurchaseReception(
              {
  ...
  private async allocateDocumentNumber(params: {
    ...
    const consecutiveNumber =
      allocation.rangeFrom + updated.currentConsecutive - 1;
  ...
  async createPendingDocumentForContingency(params: {
    ...
    if (existing) {
      ...
      if (
        !['GENERATION_ERROR', 'SIGNATURE_ERROR'].includes(existing.fiscalState)
      ) {
    ...
    await tx.fiscalDocument.create({
      data: {
        ...
        contingencyReason:
          'Offline contingency invoicing — generated by POS while disconnected from server',
        issuerNitSnapshot: issuerConfig?.nit ?? '',
        subtotal: 0,
        totalTax: 0,
        totalAmount: 0,
