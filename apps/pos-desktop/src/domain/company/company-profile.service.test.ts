...
describe("CompanyProfileService", () => {
  ...
  describe("validateDraft", () => {
    it("throws InvalidNitDvException when the verification digit does not match", () => {
      ...
      expect(() => service.validateDraft(draft)).toThrow(InvalidNitDvException);
    });
    ...
    it("throws InvalidMunicipioCodeException when the municipio code is not in the DANE catalog", () => {
      const draft = makeDraft({ municipioCode: "99999" });
    ...
    it("throws InvalidMunicipioCodeException when the municipio code has the wrong format", () => {
      const draft = makeDraft({ municipioCode: "1234" });
    ...
    it("accepts a draft whose municipio code resolves in the DANE catalog", () => {
      expect(() =>
        service.validateDraft(makeDraft({ municipioCode: "05001" })),
      ).not.toThrow();
    });
  ...
  describe("submitCompany", () => {
    ...
    it("sends the softwareId in the PATCH payload when the draft carries one", async () => {
      vi.mocked(http.patch).mockResolvedValue({});

      await service.submitCompany(makeDraft({ softwareId: "SW-42" }));

      const payload = vi.mocked(http.patch).mock
        .calls[0][1] as IssuerConfigPayload;
      expect(payload.softwareId).toBe("SW-42");
    });

    it("sends a null softwareId in the PATCH payload when the draft has none", async () => {
      vi.mocked(http.patch).mockResolvedValue({});

      await service.submitCompany(makeDraft({ softwareId: undefined }));

      const payload = vi.mocked(http.patch).mock
        .calls[0][1] as IssuerConfigPayload;
      expect(payload.softwareId).toBeNull();
    });
    ...
    it("registers the resolution on the server after a successful PATCH", async () => {
      vi.mocked(http.patch).mockResolvedValue({});
      ...
      expect(http.post).toHaveBeenCalledWith(
        `${BASE_URL}/fiscal-dian/resolutions`,
        {
          ...
          validTo: "2031-01-15T00:00:00.000Z",
        },
        { Authorization: "Bearer tok-1" },
      );
    });

    it("still completes the submit when resolution registration fails (best-effort)", async () => {
      vi.mocked(http.patch).mockResolvedValue({});
      vi.mocked(http.post).mockRejectedValue(new Error("boom"));

      await expect(service.submitCompany(makeDraft())).resolves.toBeUndefined();

      const seller = useLocalConfigStore.getState().sellerInfo;
      expect(seller.nit).toBe("900.123.456");
      expect(seller.resolutionNumber).toBe("18760000001234");
      expect(useCompanySetupStore.getState().status).toBe("complete");
    });

    it("creates the workstation allocation with the resolution id and range", async () => {
      vi.mocked(http.patch).mockResolvedValue({});
      vi.mocked(http.post)
        .mockResolvedValueOnce({ id: "res-1" })
        .mockResolvedValueOnce({});

      const workstationService = new CompanyProfileService({
        baseUrl: BASE_URL,
        httpClient: http,
        accessToken: "tok-1",
        workstationId: "ws-1",
      ...
      await workstationService.submitCompany(makeDraft());
      ...
      expect(http.post).toHaveBeenNthCalledWith(
        ...
        `${BASE_URL}/fiscal-dian/resolutions`,
        {
          ...
          validTo: new Date("2031-01-15").toISOString(),
        },
        { Authorization: "Bearer tok-1" },
      );
      expect(http.post).toHaveBeenNthCalledWith(
        2,
        `${BASE_URL}/fiscal-dian/resolution-allocations`,
        {
          resolutionId: "res-1",
          workstationId: "ws-1",
          rangeFrom: 1000,
    ...
    it("skips the allocation POST when the service has no workstation id", async () => {
      vi.mocked(http.patch).mockResolvedValue({});
      vi.mocked(http.post).mockResolvedValue({ id: "res-1" });
  ...
  describe("fetchCompanyProfile", () => {
    ...
    it("maps the active resolution and softwareId into the draft", async () => {
      vi.mocked(http.get).mockResolvedValue({
        nit: "900.123.456",
        verificationDigit: DV,
        businessName: "FARMACIA LOS ANDES S.A.S.",
      ...
      } satisfies IssuerConfigResponse);
    ...
    it("defaults the resolution prefix to FE when the server resolution has no prefix", async () => {
      vi.mocked(http.get).mockResolvedValue({
        nit: "900.123.456",
        verificationDigit: DV,
        businessName: "FARMACIA LOS ANDES S.A.S.",
      ...
      expect(draft?.resolutionPrefix).toBe("FE");
      expect(draft?.resolutionRangeStart).toBe("1");
      expect(draft?.resolutionRangeEnd).toBe("500");
...
describe("mapRegimenToTaxLevelCode", () => {
  ...
  it("maps a non-profit entity to R-99-PN-ENT", () => {
    expect(mapRegimenToTaxLevelCode("ENTIDAD SIN ÁNIMO DE LUCRO", null)).toBe(
      "R-99-PN-ENT",
    );
  });
