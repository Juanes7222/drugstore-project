...
function createFakeBackupService() {
  ...
  return {
    createBackup: vi.fn().mockResolvedValue(backupMetadata),
    listBackups: vi.fn().mockResolvedValue([]),
    verifyBackup: vi.fn().mockResolvedValue({ id: 'v1', passed: true, hashMatched: true, integrityCheckPassed: true, tableCounts: {} }),
    restoreBackup: vi.fn().mockResolvedValue({ id: 'r1', success: true, restarted: true }),
    pruneBackups: vi.fn().mockResolvedValue(0),
    getBackupSummary: vi.fn().mockResolvedValue({ lastBackupAt: null, lastBackupReason: null, totalBackups: 0, oldestBackupAt: null, totalBackupSizeBytes: 0 }),
    getBackupHealth: vi.fn().mockResolvedValue('HEALTHY'),
    uploadBackupToServer: vi.fn().mockResolvedValue({ uploadId: 'u1', workstationId: 'ws-1', createdAt: '2026-07-13T12:00:00Z' }),
    fetchLocalNumberHint: vi.fn().mockResolvedValue(null),
