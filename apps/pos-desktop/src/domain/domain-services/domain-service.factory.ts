...
import type { ImportService } from '../data-import/import.service';
...
export function createDomainServices(
  ...
  const productService = createProductService(prisma, auth);
  const clientsService = createClientsService(prisma, auth);
