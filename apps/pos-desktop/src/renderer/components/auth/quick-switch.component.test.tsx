...
describe('QuickSwitch', () => {
  ...
  it('opens the dropdown when the trigger button is clicked', () => {
    ...
    const trigger = screen.getByRole('button', {
      name: /cambiar de usuario|cambiar usuario|switch user/i,
    });
    fireEvent.click(trigger);

    // Dropdown should now show the user list
    expect(
      screen.getByText(/cambiar de usuario|cambiar usuario|switch user/i),
    ).toBeInTheDocument();
  });
  ...
  it('lists other users (excluding the current session user)', async () => {
    ...
    expect(
      await screen.findByText('Administrador del Sistema'),
    ).toBeInTheDocument();
  ...
  it('shows PIN keypad for CASHIER users (role-based credentials)', async () => {
    ...
    expect(await screen.findByLabelText('Ingrese su PIN')).toBeInTheDocument();
    expect(
  ...
  it("shows 'no permission' message on 403 when local cache is empty", async () => {
    ...
    const { loadCachedUsers } =
      await import('../../../domain/auth/local-user-cache');
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          /no tenés permisos|no permission|permisos para listar/i,
        ),
      ).toBeInTheDocument();
    });
  ...
  // -----------------------------------------------------------------------
  // Credential entry — server-reported flags (hasPin / hasPassword)
  // -----------------------------------------------------------------------

  describe('credential entry (server-reported flags)', () => {
    async function openDropdownAndSelect(user: Record<string, unknown>) {
      mockAuthService.listUsers = vi.fn().mockResolvedValue({
        users: [user],
        total: 1,
      });
      ...
      fireEvent.click(await screen.findByText(user.displayName as string));
    }

    it('shows the password panel initially when the selected user has no PIN', async () => {
      mockSessionState.session = cashierSession;
      ...
      expect(
        await screen.findByPlaceholderText('Contraseña'),
      ).toBeInTheDocument();
      expect(screen.queryByLabelText('Ingrese su PIN')).not.toBeInTheDocument();
      // No PIN credential on the server → no way back to the keypad
      expect(
        screen.queryByRole('button', { name: 'Usar PIN' }),
      ).not.toBeInTheDocument();
    });

    it('shows the keypad panel with a use-password link when the selected user has both credentials', async () => {
      mockSessionState.session = cashierSession;

      await openDropdownAndSelect({
        id: 'cashier-both',
        displayName: 'Con Ambos',
        role: 'CASHIER',
        avatarUrl: null,
        avatarColor: null,
        username: 'conambos',
