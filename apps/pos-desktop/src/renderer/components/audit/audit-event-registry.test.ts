...
describe("AUDIT_EVENT_CONFIGS", () => {
  ...
  it("has at least 49 defined events (count may grow as new events are added)", () => {
    expect(configs.length).toBeGreaterThanOrEqual(49);
  });

  it("defines the SYNC_* events written by LocalAuditWriter", () => {
    for (const action of [
      "SYNC_PUSH_COMPLETED",
      "SYNC_PUSH_FAILED",
      "SYNC_PULL_COMPLETED",
  ...
  it("defines the REPORT_* events written by LocalAuditWriter", () => {
    for (const action of [
      "REPORT_EXPORTED",
  ...
  it("maps SYNC_PUSH_FAILED to the failure category so it renders red", () => {
    expect(AUDIT_EVENT_CONFIGS.SYNC_PUSH_FAILED.category).toBe("failure");
  });

  it("maps REPORT_* events to the report category and REPORTS module", () => {
    for (const action of [
      "REPORT_EXPORTED",
      "REPORT_SHIFT_CLOSE_DOCUMENT_PERSISTED",
      "REPORT_SHIFT_CLOSE_DOCUMENT_RECOVERED",
    ]) {
      const cfg = AUDIT_EVENT_CONFIGS[action];
      expect(cfg.category).toBe("report");
      expect(cfg.module).toBe("REPORTS");
    }
  });
  ...
  it("every config has a non-empty icon string", () => {
    for (const cfg of configs) {
      ...
      expect(
        cfg.icon[0],
        `icon "${cfg.icon}" for ${cfg.action} should start with uppercase`,
      ).toBe(cfg.icon[0].toUpperCase());
    }
  });
  ...
  it("every config module is one of the 10 valid EventModule values", () => {
...
describe("EVENT_FILTER_OPTIONS", () => {
  ...
  it("excludes configs with showInFilter: false", () => {
    ...
    for (const exc of excluded) {
      ...
      expect(
        found,
        `excluded event ${exc.action} found in EVENT_FILTER_OPTIONS`,
      ).toBeUndefined();
    }
  });
  ...
  it("contains events from all 10 modules", () => {
    ...
    expect(modulesInFilter.has("REPORTS")).toBe(true);
  });
...
describe("resolveIcon", () => {
  it("returns a component for every icon name referenced in configs", () => {
    const usedIcons = new Set(
      Object.values(AUDIT_EVENT_CONFIGS).map((c) => c.icon),
    );
