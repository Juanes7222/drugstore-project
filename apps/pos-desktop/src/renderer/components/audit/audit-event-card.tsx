 ...
 *
 * Visual anatomy (per the audit redesign):
 *  1. Timeline node — colored square with the event icon, aligned with the
 *     day-group rule so cards read as one continuous rail.
 *  2. Header row — category chip (colored dot + translated name) + relative
 *     time right-aligned in font-data.
 *  3. Event name + module — event label semibold, module pill after it.
 *  4. Actor row — username in font-data + translated role badge.
 *  5. Summary fragments — human-readable detail snippets (translated).
 *  6. Target — entity name when available, else translated type + short id.
 *  7. Status — local/synced badge when `syncedAt` is present on the entry.
 *  8. Expandable panel — all parsed detail fields, 2-column grid, no raw JSON.
 *
...
const MODULE_I18N_KEY: Record<string, string> = {
  ...
  REPORTS: "audit_log.module_reports",
};
...
function isTargetMeaningless(log: AuditLogEntry): boolean {
  // entityName makes the target meaningful on its own
  if (log.entityName) return false;
...
// ---------------------------------------------------------------------------
// Stock delta chip (InventoryMovement-backed entries)
// ---------------------------------------------------------------------------

const STOCK_DELTA_ACTIONS = new Set<string>([
  "INVENTORY_SALE",
...
/**
 * The InventoryMovement reader passes quantity/previousStock/resultingStock
 * for stock-affecting events. Rendered as a monospace chip: "12 → 8 u".
 */
function buildStockDelta(
  log: AuditLogEntry,
): { from: number; to: number } | null {
  if (!STOCK_DELTA_ACTIONS.has(log.action)) return null;
  if (
    typeof log.previousStock !== "number" ||
    typeof log.resultingStock !== "number"
  ) {
    return null;
  }
  return { from: log.previousStock, to: log.resultingStock };
}
...
function parseSummaryFragments(
  ...
  try {
    ...
    // Not JSON — most likely a free-text reason from InventoryMovement.
...
function parseAllDetailFragments(
  ...
  if (parsed.quantity !== undefined) {
    ...
    fragments.push({
      key: "quantity_from",
      value:
        parsed.previousQuantity !== undefined
          ? t("audit_log.detail_quantity_from", {
              from: String(parsed.previousQuantity),
              to: String(parsed.quantity),
            })
...
export const AuditEventCard: FC<AuditEventCardProps> = ({ log }) => {
  ...
  const accentColor = getCategoryColor(log.action);
  ...
  const stockDelta = buildStockDelta(log);

  const toggleExpand = useCallback(() => {
    setExpanded((prev) => !prev);
  }, []);

  const targetLabel = log.productName
    ? `${t("audit_log.target_product")}: ${log.productName}${log.lotBatch ? ` · ${t("audit_log.detail_lot_batch", { batch: log.lotBatch })}` : ""}`
    : log.entityName
      ? `${t(ENTITY_TYPE_I18N_KEY[log.entityType ?? ""] ?? "audit_log.target_entity", { type: log.entityType ?? "" })}: ${log.entityName}`
  ...
  return (
    ...
    >
      <div className="flex gap-2.5 px-3 py-2">
        {/* ── Timeline node: icon square in category color ── */}
        <div
          ...
          aria-hidden="true"
        >
          <Icon size={16} strokeWidth={1.75} style={{ color: accentColor }} />
        </div>

        {/* ── Content column ── */}
        <div className="min-w-0 flex-1">
          {/* Row 1: category chip + event name + module + time */}
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-center gap-1.5 min-w-0 flex-wrap">
              <span
                className="inline-flex items-center gap-1 px-1.5 py-0.5 text-caption font-semibold rounded-sm shrink-0"
                style={{
                  backgroundColor: `color-mix(in srgb, ${accentColor} 10%, transparent)`,
                  color: accentColor,
                }}
              >
                {t(`audit_log.category_${config.category}`, {
                  defaultValue: t("audit_log.category_default"),
                })}
              </span>
              <span
              ...
              {config.module && (
                <span
                  className="inline-flex items-center px-1.5 py-0.5 text-caption font-medium rounded-sm shrink-0"
                  style={{
                    backgroundColor:
            ...
            <time
              className="text-caption whitespace-nowrap shrink-0 font-data"
          ...
          {/* Row 3: Stock delta chip (inventory movements) */}
          {stockDelta && (
            <div className="pt-1">
              <span
                className="inline-flex items-center gap-1 px-1.5 py-0.5 text-caption font-data rounded-sm"
                style={{
                  backgroundColor:
                    "color-mix(in srgb, var(--color-ink) 5%, transparent)",
                  color: "var(--color-ink)",
                }}
                title={t("audit_log.detail_stock_delta", {
                  from: stockDelta.from,
                  to: stockDelta.to,
                })}
              >
                {stockDelta.from} → {stockDelta.to} u
              </span>
            </div>
          )}

          {/* Row 4: Detail summary */}
          {fragments.length > 0 && (
            <div className="pt-1">
              <div
                className="flex flex-wrap gap-x-2 gap-y-0.5 text-body-sm"
                style={{ color: "var(--color-ink-muted)" }}
              >
                {fragments.map((f, i) => (
          ...
          <div className="flex items-center gap-2 pt-1.5 flex-wrap">
            {log.details && (
              <button
                ...
                aria-label={
                  expanded
                    ? t("audit_log.collapse_details")
                    : t("audit_log.expand_details")
                }
              >
                {expanded ? (
                  <ChevronDownIcon
            ...
            {log.syncedAt !== undefined && log.syncedAt !== null && (
              <span
                ...
                title={`${t("audit_log.synced_at")}: ${formatDateTime(log.syncedAt)}`}
              >
                <CloudIcon size={11} strokeWidth={1.5} aria-hidden="true" />
                {t("audit_log.synced")}
              </span>
            )}
            {log.syncedAt !== undefined && !log.syncedAt && (
              <span
                className="inline-flex items-center gap-1 text-caption"
                style={{
                  color:
                ...
                title={t("audit_log.pending_sync_hint")}
              >
                <ClockIcon size={11} strokeWidth={1.5} aria-hidden="true" />
                {t("audit_log.pending_sync")}
              </span>
            )}
          </div>
