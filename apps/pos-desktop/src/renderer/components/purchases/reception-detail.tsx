    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="pos-panel p-4 mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <span className="block text-xs text-ink-muted">{t('purchases.receptions.supplier')}</span>
            <span className="font-medium">{reception.supplier.businessName}</span>
          </div>
          {reception.purchaseOrder && (
            <div>
              <span className="block text-xs text-ink-muted">{t('purchases.receptions.purchaseOrder')}</span>
              <span className="font-data tabular-nums">#{reception.purchaseOrder.sequentialNumber}</span>
            </div>
          )}
          <div>
            <span className="block text-xs text-ink-muted">{t('purchases.receptions.items')}</span>
            <span className="font-data tabular-nums">{reception.items.length}</span>
          </div>
          <div>
            <span className="block text-xs text-ink-muted">{t('purchases.receptions.totalAmount')}</span>
            <span className="font-data tabular-nums font-bold">{formatCOP(grandTotal)}</span>
          </div>
        </div>
        {reception.notes && (
          <div className="mt-3 pt-3 border-t border-border">
            <span className="block text-xs text-ink-muted mb-1">{t('purchases.receptions.notes')}</span>
            <p className="text-sm text-ink">{reception.notes}</p>
          </div>
        )}
      </div>
        ...
        <div className="flex justify-end">
          <div className="w-64 space-y-1 text-sm">
            <div className="flex justify-between">
              <span className="text-ink-muted">{t('purchases.receptions.subtotal')}</span>
              <span className="font-data tabular-nums">{formatCOP(subtotal)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-ink-muted">{t('purchases.receptions.totalTax')}</span>
