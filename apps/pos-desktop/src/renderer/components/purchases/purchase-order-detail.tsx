    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="pos-panel p-4 mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <span className="block text-xs text-ink-muted">{t('purchases.orders.supplier')}</span>
            <span className="font-medium">{order.supplier.businessName}</span>
          </div>
          {order.expectedDeliveryDate && (
            <div>
              <span className="block text-xs text-ink-muted">{t('purchases.orders.expectedDeliveryDate')}</span>
              <span className="font-data tabular-nums">{formatShortDate(order.expectedDeliveryDate)}</span>
            </div>
          )}
          <div>
            <span className="block text-xs text-ink-muted">{t('purchases.orders.subtotal')}</span>
            <span className="font-data tabular-nums font-semibold">{formatCOP(order.subtotal)}</span>
          </div>
          <div>
            <span className="block text-xs text-ink-muted">{t('purchases.orders.total')}</span>
            <span className="font-data tabular-nums font-bold">{formatCOP(order.totalAmount)}</span>
          </div>
        </div>
        {order.notes && (
          <div className="mt-3 pt-3 border-t border-border">
            <span className="block text-xs text-ink-muted mb-1">{t('purchases.orders.notes')}</span>
      ...
      <div className="pos-panel overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="sticky top-0 z-10 border-b border-border text-left text-ink-muted text-xs uppercase tracking-wider bg-surface/50">
      ...
      {/* Timeline info */}
      <div className="mt-6 flex gap-4 text-xs text-ink-muted">
        {order.confirmedAt && (
          <span>
