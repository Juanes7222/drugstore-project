 ...
 * drains (5.5 s cycle), retelling the "sube sola" story inside the mockup.
 */
function SyncStatus() {
      ...
      <div aria-hidden="true" className="relative p-4 sm:p-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex min-w-0 flex-1 basis-44 items-center gap-2 rounded-md border border-tinta/20 bg-papel px-3 py-2 text-sm text-tinta-media">
            <ScanLineIcon className="scan-flash shrink-0 text-base text-verde-cruz" />
        ...
        {/* Cart */}
        <p className="eyebrow mt-5 text-tinta-media">
          {t("pos_preview.cart_title")}
        </p>
        <ul className="mt-2 divide-y divide-tinta/10 border-y border-tinta/10">
          {ITEMS.map((item) => (
            <li key={item.nameKey} className="py-3">
              ...
              {item.nearExpiry && (
                <p className="data mt-1.5 inline-flex items-center gap-1.5 rounded bg-ambar-lote-fondo px-2 py-0.5 text-[11px] font-medium text-ambar-lote-texto">
        ...
        >
          ...
          <span className="data rounded bg-white/20 px-1.5 py-0.5 text-[11px]">
            F2
          </span>
        </button>
      </div>
